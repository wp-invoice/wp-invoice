...
/**
  Name: Conekta
  Class: wpi_conekta
  Internal Slug: wpi_conekta
  JS Slug: wpi_conekta
  Version: 1.0
  Description: Provides Conekta gateway
 */
class wpi_conekta extends wpi_gateway_base {
    ...
    $this->options = array(
        ...
        'settings' => array(
            'test_secret_key' => array(
                'label' => __( "Test Private Key", ud_get_wp_invoice()->domain ),
                'value' => ''
            ),
            'test_publishable_key' => array(
                'label' => __( "Test Public Key", ud_get_wp_invoice()->domain ),
                'value' => ''
            ),
            'live_secret_key' => array(
                'label' => __( "Live Private Key", ud_get_wp_invoice()->domain ),
                'value' => ''
            ),
            'live_publishable_key' => array(
                'label' => __( "Live Public Key", ud_get_wp_invoice()->domain ),
                'value' => ''
            ),
            'mode' => array(
                'label' => __("Mode", ud_get_wp_invoice()->domain),
                'description' => "Conekta payment mode",
                'type' => 'select',
                'value' => 'test',
                'data' => array(
                    'test' => "Test",
            ...
            'webhook_url' => array(
                ...
                'type' => "readonly",
                'value' => admin_url('admin-ajax.php?action=wpi_gateway_server_callback&type=wpi_conekta'),
                'description' => __( "Use webhooks to be notified about events that happen in your Conekta account.", ud_get_wp_invoice()->domain )
            )
        )
    );
  ...
  function recurring_settings($this_invoice) {
      ?>
      <h4><?php _e('Conekta Subscriptions', ud_get_wp_invoice()->domain); ?></h4>
      <table class="wpi_recurring_bill_settings">
          ...
          <tr>
              ...
              <td>
                  <?php echo WPI_UI::input("id=conekta_interval_count&name=wpi_invoice[recurring][".$this->type."][interval_count]&value=" . (!empty($this_invoice['recurring'][$this->type]) ? $this_invoice['recurring'][$this->type]['interval_count'] : '') . "&special=size='2' maxlength='4' autocomplete='off'"); ?>
              </td>
          </tr>
      </table>
      <?php
        ...
        if (!empty($this->front_end_fields[$key])) {
          ...
          <ul class="wpi_checkout_block">
            ...
            foreach ($value as $field_slug => $field_data) {
              ...
              $field_data = apply_filters('wpi_payment_form_styles', $field_data, $field_slug, 'wpi_conekta');
              $html = '';
    ...
    static function process_payment() {
      ...
      try {
        ...
        if ( !class_exists('Conekta') ) {
          require_once( ud_get_wp_invoice()->path( "lib/third-party/conekta/lib/Conekta.php", 'dir' ) );
        }
        $pk = trim($invoice['billing']['wpi_conekta']['settings'][$invoice['billing']['wpi_conekta']['settings']['mode']['value'].'_secret_key']['value']);

        \Conekta\Conekta::setApiKey($pk);
        ...
        switch( $invoice['type'] == 'recurring' ) {
          ...
          case true:
            ...
            $plan = \Conekta\Plan::create(array(
              ...
              "interval" => $invoice['recurring']['wpi_conekta']['interval'],
              "frequency" => $invoice['recurring']['wpi_conekta']['interval_count'],
            ...
            if ( !empty( $plan->id ) && !empty( $plan->amount ) && !empty( $customer->id ) ) {
              ...
              update_post_meta( wpi_invoice_id_to_post_id( $invoice['invoice_id'] ), '_conekta_customer_id', $customer->id );

              $data['messages'][] = __( 'Conekta Subscription has been initiated. Do not pay this invoice again. Thank you.', ud_get_wp_invoice()->domain );
              $response['success'] = true;
              $response['error'] = false;
            } else {

              $data['messages'][] = __( 'Could not initiate Conekta Subscription. Contact site Administrator please.', ud_get_wp_invoice()->domain );
          ...
          case false:
            ...
            $charge = \Conekta\Charge::create(array(
              ...
              "currency" => strtolower( $invoice['default_currency_code'] ),
              "card" => $token,
              "description" => $invoice['invoice_id'].' ['.$invoice['post_title'].' / '.get_bloginfo('url').']'
              "details"=> array(
                  "name"  => $_REQUEST['first_name'].' '.$_REQUEST['last_name'],
                  "phone" => $_REQUEST['phonenumber']
                  "email"=> $invoice['user_email'],
            ...
            if ( $charge->paid ) {
              ...
              //** Add payment amount */
              $event_note = WPI_Functions::currency_format((float)$amount, $invoice['invoice_id']) . __(" paid via CONEKTA", ud_get_wp_invoice()->domain);
              $event_amount = $amount;
    ...
    static function server_callback() {
      ...
      switch ($event_object->type) {
        ...
        case 'subscription.paid':
          ...
          FROM {$wpdb->postmeta}
          WHERE meta_key = '_conekta_customer_id'
          ...
          $pk = trim($invoice_object->data['billing']['wpi_conekta']['settings'][$invoice_object->data['billing']['wpi_conekta']['settings']['mode']['value'] . '_secret_key']['value']);

          \Conekta\Conekta::setApiKey($pk);

          $event = \Conekta\Event::find($event_object->id);


          if ($event->data->object->paid == 1) {
            $event_amount = (float) ($event->data->object->amount / 100);
            $event_note = WPI_Functions::currency_format(abs($event_amount), $invoice_object->data['invoice_id']) . ' ' . __('Conekta Subscription Payment', ud_get_wp_invoice()->domain);
            $event_type = 'add_payment';
        ...
        case 'subscription.canceled':
          ...
          if ( $event->data->object->status == 'canceled' ) {
            $invoice_object->add_entry("attribute=invoice&note=".__('Conekta Subscription has been canceled', ud_get_wp_invoice()->domain)."&type=update");
            $invoice_object->save_invoice();
