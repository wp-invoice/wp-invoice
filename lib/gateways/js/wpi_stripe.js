...
var wpi_stripe_submit = function(){
  ...
  expMonth = jQuery('.card-expiry-month').val(),
  expYear = jQuery('.card-expiry-year').val();
  var infoWrapper = jQuery('.wpi_checkout_block.customer_information');
  var name = infoWrapper.find('[name=first_name]').val() + ' ' + infoWrapper.find('[name=last_name]').val();
  ...
  try {
    Stripe.createToken({
      ...
      exp_year:          expYear,
      name:              name.trim(),
      address_line1:     infoWrapper.find('[name=address1]').val(),
      address_line2:     '', 
      address_city:      infoWrapper.find('[name=city]').val(),
      address_state:     infoWrapper.find('[name=state]').val(),
      address_zip:       infoWrapper.find('[name=zip]').val(),
      address_country:   infoWrapper.find('[name=country]').val(),
