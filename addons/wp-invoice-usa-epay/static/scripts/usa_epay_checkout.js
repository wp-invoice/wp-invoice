/**
 * WPI USA EPAY. Single Page Checkout
 *
 * @author korotkov@UD
 */

jQuery( document ).on( 'wpi_checkout_init-wpi_usa_epay', function( event, self ) {

  if( typeof self.params.available_gateways.wpi_usa_epay === 'undefined' ) {
    return false;
  }

  var i = self.params.available_gateways.wpi_usa_epay.data;
   ...
   */
  self.gateways.wpi_usa_epay = {
    amount:0,
    wpi_checkout_total:0,
    extended_validation_passed:true,
    submit_function: function() {
      self.update_checkboxes( self.gateways.wpi_usa_epay );
      var wpi_checkout_form = this;
      // if venue has null amount
      if( self.gateways.wpi_usa_epay.wpi_checkout_total == 0 ){
        return false;
      }
      // disable UI
      ...
      jQuery.post( i.ajaxurl + '?wpi_timestamp=' + i.time, {
        ...
        data: data
            ...
            if( result.payment_status == 'validation_fail') {
              ...
              //** Trigger added to be able to hook it to add some custom stuff @author korotkov@UD */
              jQuery(document).trigger('wpi_spc_success', [result, wpi_checkout_form, 'wpi_usa_epay']);
  ...
  if( jQuery( '.wpi_checkout_payment_amount_input', self.instance ).length > 0 ){
    self.gateways.wpi_usa_epay.amount = parseFloat(jQuery('.wpi_checkout_payment_amount_input', self.instance ).val());
    self.update_checkboxes( self.gateways.wpi_usa_epay );
  }
  ...
  jQuery('.wpi_checkout_payment_amount_input', self.instance ).keyup( function(){
    self.gateways.wpi_usa_epay.amount = parseFloat(jQuery(this).val());
    self.update_checkboxes( self.gateways.wpi_usa_epay );
  });
  ...
  if( typeof jQuery.fn.form_helper == 'function' ) {
    jQuery('form.wpi_usa_epay', self.instance ).bind( 'form_helper::success', function(){
      jQuery(this).unbind().submit( self.gateways.wpi_usa_epay.submit_function ).submit();
    });
  } else {
    jQuery('form.wpi_usa_epay', self.instance ).submit( self.gateways.wpi_usa_epay.submit_function );
  }
