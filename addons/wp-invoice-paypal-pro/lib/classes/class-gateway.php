...
/**
  Name: PayPal Pro
  Class: wpi_paypal_pro
  Internal Slug: wpi_paypal_pro
  JS Slug: wpi_paypal_pro
  Version: 1.0
  Description: Provides PayPal Payments Pro gateway addon
 */

namespace UsabilityDynamics\WPI_PPP {

  if (!class_exists('UsabilityDynamics\WPI_PPP\Gateway') && class_exists('\wpi_gateway_base')) {
    ...
    class Gateway extends \wpi_gateway_base {
      ...
      function __construct() {
        ...
        $this->front_end_fields = array(
          'customer_information' => array(
            'first_name' => array(
              ...
              'class' => 'text-input',
              'name' => 'first_name',
              'label' => __('First Name', ud_get_wp_invoice_paypal_pro()->domain)
            ),
            'last_name' => array(
              'type' => 'text',
              'class' => 'text-input',
              'name' => 'last_name',
              'label' => __('Last Name', ud_get_wp_invoice_paypal_pro()->domain)
            ),
            'user_email' => array(
              'type' => 'text',
              'class' => 'text-input',
              'name' => 'email',
              'label' => __('Email Address', ud_get_wp_invoice_paypal_pro()->domain)
            ),
            'phonenumber' => array(
              'type' => 'text',
              'class' => 'text-input',
              'name' => 'phone',
              'label' => __('Phone', ud_get_wp_invoice_paypal_pro()->domain)
            ),
            'streetaddress' => array(
              'type' => 'text',
              'class' => 'text-input',
              'name' => 'street',
              'label' => __('Street', ud_get_wp_invoice_paypal_pro()->domain)
            ),
            'city' => array(
              'type' => 'text',
              'class' => 'text-input',
              'name' => 'city',
              'label' => __('City', ud_get_wp_invoice_paypal_pro()->domain)
            ),
            'zip' => array(
              'type' => 'text',
              'class' => 'text-input',
              'name' => 'zip',
              'label' => __('Zip/Postal Code', ud_get_wp_invoice_paypal_pro()->domain)
            ),
            'country' => array(
              'type' => 'countries',
              'class' => 'text-input',
              'name' => 'countrycode',
              'label' => __('Country', ud_get_wp_invoice_paypal_pro()->domain)
            ),
            'state' => array(
              'type' => 'states',
              'class' => 'text-input',
              'name' => 'state',
              'label' => __('State/Province', ud_get_wp_invoice_paypal_pro()->domain)
            )
          )
        );
        ...
        $this->options = array(
          ...
          'settings' => array(
            'api_signature' => array(
              'label' => __("API Signature", ud_get_wp_invoice_paypal_pro()->domain),
              'value' => ''
            ),
            'api_username' => array(
              'label' => __("API Username", ud_get_wp_invoice_paypal_pro()->domain),
              'value' => ''
            ),
            'api_password' => array(
              'label' => __("API Password", ud_get_wp_invoice_paypal_pro()->domain),
              'value' => ''
            ),
            'test_mode' => array(
              'label' => __("Use in Test Mode", ud_get_wp_invoice_paypal_pro()->domain),
              'description' => __("Use PayPal SandBox for test mode", ud_get_wp_invoice_paypal_pro()->domain),
              'type' => 'select',
              'value' => 'N',
              'data' => array(
                'N' => __("No", ud_get_wp_invoice_paypal_pro()->domain),
                'Y' => __("Yes", ud_get_wp_invoice_paypal_pro()->domain)
              )
            )
          )
        );
      ...
      function frontend_display($args = '', $from_ajax = false) {
        ...
        include( ud_get_wp_invoice_paypal_pro()->path('static/views/', 'dir') . 'paypal_pro_frontend.php' );
      ...
      function recurring_settings($invoice) {
        ?>
        <h4><?php _e('PayPal Pro Recurring Billing', ud_get_wp_invoice_paypal_pro()->domain); ?></h4>
        <p><?php _e('Currently PayPal Pro gateway does not support Recurring Billing', ud_get_wp_invoice_paypal_pro()->domain); ?></p>
        <?php
      }
      ...
      function wpi_payment_fields($invoice) {
          ...
          foreach ($this->front_end_fields as $key => $value) {
            ...
            if (!empty($this->front_end_fields[$key])) {
              ...
              <ul class="wpi_checkout_block">
                <li class="section_title"><?php _e(ucwords(str_replace('_', ' ', $key)), ud_get_wp_invoice_paypal_pro()->domain); ?></li>
                <?php
                $html = ob_get_clean();
                echo $html;
                ...
                foreach ($value as $field_slug => $field_data) {
                  ...
                  switch ($field_data['type']) {
                    case self::TEXT_INPUT_TYPE:
                      ...
                      <li class="wpi_checkout_row">
                        <div class="control-group">
                          <label class="control-label" for="<?php echo esc_attr($field_slug); ?>"><?php _e($field_data['label'], ud_get_wp_invoice_paypal_pro()->domain); ?></label>
                          <div class="controls">
                    ...
                    case self::SELECT_STATES_TYPE:
                      ?>

                      <li class="wpi_checkout_row">
                        <label class="control-label" for="<?php echo esc_attr($field_slug); ?>"><?php _e($field_data['label'], ud_get_wp_invoice_paypal_pro()->domain); ?></label>
                        <select name="state" id="state" class="text-input">
                          <option value=""><?php _e('No', ud_get_wp_invoice_paypal_pro()->domain); ?></option>
                          <?php foreach (HelperCodes::$states as $state_area => $area_states): ?>
                            <optgroup label="<?php echo $state_area; ?>">
                              <?php foreach ($area_states as $state_key => $state_label): ?>
                                <option value="<?php echo $state_key; ?>"><?php echo $state_label; ?></option>
                              <?php endforeach; ?>
                            </optgroup>
                          <?php endforeach; ?>
                        </select>
        ...
        static function process_payment() {
          ...
          $gateway_settings = $invoice['billing']['wpi_paypal_pro']['settings'];
          $invoice_id = $invoice['invoice_id'];

          try {

            $request = new PayPalDirectPayment(
                $gateway_settings['api_username']['value'], $gateway_settings['api_password']['value'], $gateway_settings['api_signature']['value'], $gateway_settings['test_mode']['value'] === 'Y' ? true : false
            );

            $request->Amount         = $_REQUEST['amount'];
            $request->CardType       = $_REQUEST['credit_card_type'];
            $request->CardNumber     = $_REQUEST['acct'];
            $request->CardExpiration = $_REQUEST['exp_m'] . $_REQUEST['exp_y'];
            $request->CardCVV2       = $_REQUEST['cvv2'];
            $request->FirstName      = $_REQUEST['first_name'];
            $request->LastName       = $_REQUEST['last_name'];
            $request->Address        = $_REQUEST['street'];
            $request->City           = $_REQUEST['city'];
            $request->State          = $_REQUEST['state'];
            $request->Zip            = $_REQUEST['zip'];
            $request->Country        = $_REQUEST['countrycode'];
            $request->Email          = $_REQUEST['email'];
            $request->Currency       = $_REQUEST['currency_code'];
            $request->Description    = $invoice['post_title'];
            $request->InvoiceNumber  = $invoice_id;

            $request->send();

            $response['gateway_response'] = $request->Response;
            ...
            if ($request->Response['ACK'] === 'Success') {
              ...
              $amount = (float)$request->Response['AMT'];
              
              //** Add payment amount */
              $event_note = \WPI_Functions::currency_format($amount, $invoice['invoice_id']) . __(" paid via PayPal Direct Payment (Pro)", ud_get_wp_invoice_paypal_pro()->domain);
              $event_amount = $amount;
              ...
              //** Log client IP */
              $success = __("Successfully processed by ", ud_get_wp_invoice_paypal_pro()->domain).$_SERVER['REMOTE_ADDR'];
              $invoice_obj->add_entry("attribute=invoice&note=$success&type=update");
              
              //** Log payer */
              $payer_card = __("PayPal Transaction ID: ", ud_get_wp_invoice_paypal_pro()->domain).$request->Response['TRANSACTIONID'];
              ...
              $data['messages'][] = __( 'Successfully paid. Thank you.', ud_get_wp_invoice_paypal_pro()->domain );
              $response['success'] = true;
              $response['error'] = false;
              
            } else {
              
              $data['messages'][] = $request->Response['L_LONGMESSAGE0'];
