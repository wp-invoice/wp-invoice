...
namespace UsabilityDynamics\MijirehClient {
  ...
  if (!class_exists('\UsabilityDynamics\MijirehClient\Rest')) {
    class Rest
    {
      ...
      protected function checkLastResponseForError()
      {
        ...
        switch ($meta['http_code']) {
          case 400:
            throw new Rest_BadRequest($this->processError($body));
            break;
          case 401:
          ...
          case 404:
            ...
            break;
          case 405:
            throw new Rest_MethodNotAllowed($this->processError($body));
            break;
          case 409:
            throw new Rest_Conflict($this->processError($body));
            break;
          case 410:
            throw new Rest_Gone($this->processError($body));
            break;
          case 422:
            ...
            throw new Rest_InvalidRecord($this->processError($body));
            break;
          default:
            if ($meta['http_code'] >= 400 && $meta['http_code'] <= 499)
              throw new Rest_ClientError($this->processError($body));
            elseif ($meta['http_code'] >= 500 && $meta['http_code'] <= 599)
              throw new Rest_ServerError($this->processError($body));
            elseif (!$meta['http_code'] || $meta['http_code'] >= 600) {
              throw new Rest_UnknownResponse($this->processError($body));
            }
        }
      }
    }
    ...
    class Rest_UnknownResponse extends Rest_Exception
    {
    }

    /* 401-499 */

    class Rest_ClientError extends Rest_Exception
    {
    }

    /* 400 */

    class Rest_BadRequest extends Rest_ClientError
    {
    }

    /* 401 */

    class Rest_Unauthorized extends Rest_ClientError
    {
    }

    /* 403 */

    class Rest_Forbidden extends Rest_ClientError
    {
    }

    /* 404 */

    class Rest_NotFound extends Rest_ClientError
    {
    }

    /* 405 */

    class Rest_MethodNotAllowed extends Rest_ClientError
    {
    }

    /* 409 */

    class Rest_Conflict extends Rest_ClientError
    {
    }

    /* 410 */

    class Rest_Gone extends Rest_ClientError
    {
    }

    /* 422 */

    class Rest_InvalidRecord extends Rest_ClientError
    {
    }

    /* 500-599 */

    class Rest_ServerError extends Rest_Exception
    {
    }
  }
}