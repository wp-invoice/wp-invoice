...
<form method="POST" class="<?php echo apply_filters('wpi_spc::form_class', "wpi_checkout {$gateway_key}"); ?>" action="#" <?php echo $display ? '' : 'style="display:none;"'; ?>>
  ...
  <?php
  add_filter('wpi_spc::input_attributes', array( 'UsabilityDynamics\WPI_PPP\Gateway', 'input_attributes' ), 10, 2);
  ?>

  <!-- CUSTOMER INFORMATION -->
  <ul class="wpi_checkout_block wpi_checkout_customer_information">
    <li class="section_title"><?php _e( 'Customer Information', ud_get_wp_invoice_mijireh_checkout()->domain ); ?></li>
    <?php foreach($wpi_checkout['info_block']['customer_information'] as $slug => $data): ?>
      <li class="wpi_checkout_row_<?php echo $slug; ?> wpi_checkout_row">
        ...
        <div <?php echo $attrs; ?> class="<?php echo apply_filters('wpi_spc::group_coltrol_class', 'control-group'); ?>">
          <?php					$input_classes = implode(' ', apply_filters('wpi_spc::form_input_classes', array("input-large", "text-input", "wpi_checkout_payment_{$slug}_input"), $slug, $data, 'wpi_mijireh_checkout') ); ?>
  ...
  <ul class="wpi_checkout_block wpi_checkout_billing_address">
    <li class="section_title"><?php _e( 'Billing Address', ud_get_wp_invoice()->domain ); ?></li>

    <li class="wpi_checkout_row_street wpi_checkout_row">
      <div class="control-group">
        <label class="control-label" for="wpi_checkout_payment_street">
          <?php _e( 'Street', ud_get_wp_invoice_mijireh_checkout()->domain ); ?>
        </label>
        <div class="controls">
    ...
    <li class="wpi_checkout_row_city wpi_checkout_row">
      <div class="control-group">
        <label class="control-label" for="wpi_checkout_payment_city">
          <?php _e( 'City', ud_get_wp_invoice_mijireh_checkout()->domain ); ?>
        </label>
        <div class="controls">
    ...
    <li class="wpi_checkout_row_zip wpi_checkout_row">
      <div class="control-group">
        <label class="control-label" for="wpi_checkout_payment_zip">
          <?php _e( 'Zip/Postal Code', ud_get_wp_invoice_mijireh_checkout()->domain ); ?>
        </label>
        <div class="controls">
    ...
    <li class="wpi_checkout_row_state wpi_checkout_row">
      <div class="control-group">
        <label class="control-label" for="wpi_checkout_payment_state">
          <?php _e( 'State', ud_get_wp_invoice_mijireh_checkout()->domain ); ?>
        </label>
        <div class="controls">
    ...
    <li class="wpi_checkout_row_state wpi_checkout_row">
      <div class="control-group">
        <label class="control-label" for="wpi_checkout_payment_country">
          <?php _e( 'Country', ud_get_wp_invoice_mijireh_checkout()->domain ); ?>
        </label>
        <div class="controls">
  ...
  <ul class="wpi_checkout_block wpi_checkout_billing_information">
    <li class="section_title"><?php _e( 'Amount', ud_get_wp_invoice_usa_epay()->domain ); ?></li>
    <li class="wpi_checkout_row_amount wpi_checkout_row">
      <?php
      $group_control_attrs = apply_filters('wpi_spc::group_control_attrs', array(), 'amount', $wpi_checkout['info_block']['billing_information']['amount']);
      $attrs = '';
      if ( !empty( $group_control_attrs ) && is_array( $group_control_attrs ) )
        foreach ($group_control_attrs as $attr_name => $attr_value) {
          $attrs .= $attr_name.'="'.$attr_value.'" ';
      ...
      ?>
      <div <?php echo $attrs; ?> class="<?php echo apply_filters('wpi_spc::group_coltrol_class', 'control-group'); ?>">
        <?php					$input_classes = implode(' ', apply_filters('wpi_spc::form_input_classes', array("input-large", "text-input", "wpi_checkout_payment_amount_input"), 'amount', $wpi_checkout['info_block']['billing_information']['amount'], 'wpi_mijireh_checkout') ); ?>
        <?php         $value = !empty($atts['amount'])?$atts['amount']:''; ?>
        <?php					echo apply_filters("wpi_checkout_input_amount", "<label class='control-label' for='wpi_checkout_payment_amount'>{$wpi_checkout['info_block']['billing_information']['amount']['label']}</label><div class='controls'><input ".implode(' ', apply_filters('wpi_spc::input_attributes', array("type='text'"),'amount'))." name='wpi_checkout[billing][amount]' value='".apply_filters('wpi_spc::input_value', esc_attr($value), 'amount', $atts)."' id='wpi_checkout_payment_amount_{$gateway_key}' class='{$input_classes}' /><span class='help-inline validation'></span></div>", 'amount', $wpi_checkout['info_block']['billing_information']['amount']); ?>
      </div>
    </li>
  </ul>
  <?php endif; ?>
  ...
  <div class="<?php echo apply_filters('wpi_spc::form_actions_class', 'form-actions', 'wpi_mijireh_checkout'); ?>">
    <?php $checkout_button_classes = implode(' ', apply_filters('wpi_spc::checkout_button_classes', array("btn", "btn-success", "wpi_checkout_process_payment", "wpi_mijireh_checkout"))); ?>
