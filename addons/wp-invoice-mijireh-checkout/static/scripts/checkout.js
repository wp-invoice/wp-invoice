/**
 * Mijireh Checkout. Single Page Checkout
 *
 * @author korotkov@UD
 */

jQuery( document ).on( 'wpi_checkout_init-wpi_mijireh_checkout', function( event, self ) {

  if( typeof self.params.available_gateways.wpi_mijireh_checkout === 'undefined' ) {
    return false;
  }

  var i = self.params.available_gateways.wpi_mijireh_checkout.data;
   ...
   */
  self.gateways.wpi_mijireh_checkout = {
    amount:0,
    wpi_checkout_total:0,
    extended_validation_passed:true,
    submit_function: function() {
      self.update_checkboxes( self.gateways.wpi_mijireh_checkout );
      var wpi_checkout_form = this;
      // if venue has null amount
      if( self.gateways.wpi_mijireh_checkout.wpi_checkout_total == 0 ){
        return false;
      }
      // disable UI
      ...
      jQuery.post( i.ajaxurl + '?wpi_timestamp=' + i.time, {
        ...
        data: data
            ...
            if( result.payment_status == 'validation_fail') {
              ...
              //** Trigger added to be able to hook it to add some custom stuff @author korotkov@UD */
              jQuery(document).trigger('wpi_spc_success', [result, wpi_checkout_form, 'wpi_mijireh_checkout']);
  ...
  if( jQuery( '.wpi_checkout_payment_amount_input', self.instance ).length > 0 ){
    self.gateways.wpi_mijireh_checkout.amount = parseFloat(jQuery('.wpi_checkout_payment_amount_input', self.instance ).val());
    self.update_checkboxes( self.gateways.wpi_mijireh_checkout );
  }
  ...
  jQuery('.wpi_checkout_payment_amount_input', self.instance ).keyup( function(){
    self.gateways.wpi_mijireh_checkout.amount = parseFloat(jQuery(this).val());
    self.update_checkboxes( self.gateways.wpi_mijireh_checkout );
  });
  ...
  if( typeof jQuery.fn.form_helper == 'function' ) {
    jQuery('form.wpi_mijireh_checkout', self.instance ).bind( 'form_helper::success', function(){
      jQuery(this).unbind().submit( self.gateways.wpi_mijireh_checkout.submit_function ).submit();
    });
  } else {
    jQuery('form.wpi_mijireh_checkout', self.instance ).submit( self.gateways.wpi_mijireh_checkout.submit_function );
  }
