...
class wpi_chargify {
          ...
          if ( $this->valid_api_key ) {
            ...
            if ( !$products ) {
              ...
              <a id="wpi_ajax_check_chargify_products" href="#"
                 class="button"><?php _e( 'Check for Chargify Products', WPI ); ?></a> <?php
              ...
              <table class="form-table">
                <tr class="wpi_something_advanced_wrapper">
                  ...
                  <td><a id="wpi_ajax_check_chargify_products" href="#" class="button"><?php _e( 'Go', WPI ); ?></a>
                  </td>
                </tr>
                <tr class="wpi_something_advanced_wrapper">
                        ...
                        foreach ( $products as $product ) {
                          ...
                          foreach ( $components[ $product->product_family->id ] as $component ) {
                            ...
                            <tr class="component <?php echo $alternate; ?>">
                              <td><?php echo $product->product_family->name; ?>
                                - <?php echo $component[ 'name' ]; ?></td>
        ...
        <?php if ( $atts[ 'show_components' ] == 'true' && $components ): ?>
          ...
          <dl>
            <?php foreach ( $components as $c ): ?>
              ...
              <dd>
                <?php echo wpi_chargify::get_component_price_line( $c ); ?>
                / <?php echo wpi_chargify::get_component_type_line( $c ); ?> Charging
              </dd>
            <?php endforeach; ?>
          </dl>
        <?php endif; ?>
  ...
  function shortcode_frontend_manage( $default_atts = "", $content = null, $code = "" ) {
    ...
    try {
                ...
                <td>
                  <div class="title-row">
                      <span class="actions">
                        ...
                        <a class="cancel" for_subscription="<?php echo $sub->id; ?>"
                           href="#"><?php _e( 'Cancel', WPI ); ?></a>
                      </span>
              ...
              foreach ( $products as $product ) {
                ...
                <tr for_product="<?php echo $product->handle; ?>">
                  <td>
                    <div class="title-row">
                      <span class="actions">
                        <a href="#" for_product="<?php echo $product->handle; ?>"
                           action="subscribe"><?php _e( 'Subscribe', WPI ); ?></a> <?php
                        if ( $product->description ) {
                          ?>
                          | <a href="#" for_product="<?php echo $product->handle; ?>"
                               action="more-info"><?php _e( 'More Info', WPI ); ?></a> <?php
                        } ?>
                      </span>
      ...
      <style type="text/css">
        .wpi_chargify_subs_wrap div.title-row {
          position: relative;
        }
        ...
        .wpi_chargify_subs_wrap .product {
          width: 30%;
        }

        .wpi_chargify_subs_wrap .cost {
          width: 25%;
        }
  ...
  function get_price_line( $product ) {
    return '$' . wp_invoice_currency_format( $product->price_in_cents / 100 ) . ' for ' . $product->interval . ' ' . $product->interval_unit . '(s)';
  }

  /** Gets our properly formatted header */
  function get_product_title( $product ) {
    return $product->product_family->name . ' - ' . $product->name;
  }
  ...
  function get_last_four( $card ) {
    return substr( str_ireplace( '-', '', $card ), -4 );
  }

  /** State of the product */
  function format_product_sate( $state ) {
